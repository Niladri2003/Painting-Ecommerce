...
const ProductCard = ({ product, showDiscountPercentage,showOriginalPrice }) => {
    ...
    return (
            ...
            <div className="product-card relative  rounded-lg lg:p-10 p-9">
                ...
                <div className="image-container">
                        ...
                        <span   onClick={toggleFavorite}
                            ...
                            {isFavorited ? <FaHeart color={"black"} size={28}/> : <FaRegHeart color={"black"} size={28}/>}
                        </span>

                    <img
                        src={product.images[0]?.image_url || 'default-image.jpg'}
                        alt={product.title}
                        className="rounded-t-lg hover:scale-[1.02] transition-transform duration-300"
                    />
                </div>
    ...
    <Link to={`/product/${product.id}`}>
                <div className="product-details mt-4 font-Poppins flex flex-row justify-between items-center">
                    <div className={"flex flex-col"}>
                       ...
                       <div className={"flex flex-row gap-2 items-center"}> <p className="text-gray-800 font-semibold">
                           ₹{product.discounted_price}
                       </p>
                           {
                               discountPercentage > 0 && (
                                   <p className={" text-red-600 font-Poppins lg:text-md text-[10px]"}>{discountPercentage}% Off</p>
                               )
                           }</div>
                    </div>
                    <span onClick={toggleFavorite}>
