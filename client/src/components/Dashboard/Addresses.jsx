      ...
      <ul className="mt-4 space-y-4 overflow-y-auto max-h-[35rem]l scrollbar-thin">
        ...
        {addresses?.length > 0 ? (
          addresses.map((address) =>{  
            ...
            <li
              key={address.id}
              className="bg-white py-4 px-6 rounded-md  flex flex-col md:flex-row justify-between items-end border-2 shadow-sm gap-3 md:gap-0"
            >
              <div>
                <p className="italic text-2xl text-gray-300 capitalize font-semibold">{address.set_as_default&&"default address"}</p>
              ...
              <div className="flex flex-col gap-2 self-start  w-[10.6rem] md:h-[9.9rem]">
                <div className="p-1 rounded-full bg-black cursor-pointer w-max md:self-end"
              onClick={() => toggleOpen(address.id)}
              >
                {!openStates[address.id]?<HiDotsCircleHorizontal
                ...
                {openStates[address.id]&&<div className="flex space-x-3 self-start bg-gray-100 px-5 py-5">
