...
const Orders = () => {
    ...
    const handleCardClick = (order_id) => {
        navigate(`/account/order-details/${order_id}`);
    };
    return (
                ...
                <Tbody>
                    {orders && orders.data.map((order) => (
                        ...
                        {/*<Link to={`/account/order-details/${order.order_id}`} key={order?.order_id}>*/}
                            <Tr  onClick={()=>(handleCardClick(order?.order_id))} >
                                <Td>{order?.order_items[0]?.product_name}</Td>
                                <Td>₹{order?.total}</Td>
                                <Td><span className={`font-bold uppercase ${order.order_status === 'Delivered' ? 'text-green-700' : 'text-yellow-700'}`}>{order.order_status}</span></Td>
